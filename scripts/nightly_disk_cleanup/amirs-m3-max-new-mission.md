Run $disk-cleanup as the authorized nightly maintainer on amirs-m3-max-new.
...
above target too. Read the most recent prior run's summary.json and actions.jsonl
under ~/.local/state/disk-cleanup/runs/ (latest points at the current run).
Reuse the known owner list and refresh activity/retention checks. When already
above target, finish routine expiry/cache housekeeping without a new full
owner survey. Routine retention: remove old rotated diagnostic logs and recognized timestamped
...
have host: "amirs-m3-max-new", status: "ok", "warning" or "failed", and volumes: a
