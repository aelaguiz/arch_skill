...
- `fresh-consult` — prompt-only fresh Claude Opus, Codex GPT/GBT, or Cursor Composer second opinions for cold reads, parallel consults, flow consistency audits, completion checks, and readability/confusion checks; asks once for runtime/model/effort when missing and reports each child result back to the parent skill
...
Use `code-review` when the user wants an automated finding-set with explicit coverage guarantees. Use `plan-audit implementation-audit` when the user wants prompt-first code review against a specific plan without running tests, proving CI, or launching the code-review runner. Use `fresh-consult` when the user wants a general Claude Opus, Codex GPT/GBT, or Cursor Composer second opinion without the code-review runner. Use `codex-review-yolo` when the user wants a narrower, more interactive `-p yolo` fresh-eyes consult on a specific artifact.
