---
name: arch-epic
description: "Orchestrate a large multi-plan goal into ordered sub-plans by wrapping `$arch-step`: capture the goal, propose a one-sentence-per-sub-plan decomposition, get user approval, then for each sub-plan in order invoke `$arch-step new` → `auto-plan` → `implement-loop` → `audit-implementation` and run a fresh external critic that checks for scope drift before advancing. Progressive lazy planning — sub-plan N+1's planning only starts after sub-plan N is complete and passes the critic. Use when the user has a goal too big for one `$arch-step` run and wants user-approved decomposition, progressive North-Star approval, scope-drift detection between sub-plans, and resumable orchestration that picks up where it left off from on-disk state alone (\"continue my epic\", \"pick up where we left off\", \"keep going on <project>\"). Do NOT use for: a single architecture plan (use `$arch-step`), one-pass mini plans (use `$arch-mini-plan`), 1–3 phase features (use `$lilarch`), open-ended optimization loops (use `$goal-loop`), read-only status routing (use `$arch-flow`), or stepped orchestration in a foreign repo (use `$stepwise`)."
metadata:
