...
Canonical mini-plan artifact, required blocks, status rules, and handoff contract. Core doctrine lives in references/arch-operating-doctrine.md and references/controller-lifecycle.md. This document owns only the mini-plan-specific artifact contract.

## DOC_PATH

Reuse an existing `docs/<...>.md` when the user gives one or when repo evidence makes the active plan obvious.
Otherwise create a new canonical plan doc under `docs/`: `docs/<TITLE_SCREAMING_SNAKE>_<YYYY-MM-DD>.md`
...
- YAML frontmatter with: `title`, `date`, `status`, `fallback_policy`, `owners`, `reviewers`, `doc_type`, `related`
- a filled `# TL;DR`
- a filled North Star with: claim, in scope, out of scope, definition of done, invariants
