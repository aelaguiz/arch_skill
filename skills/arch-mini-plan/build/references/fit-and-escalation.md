...
Concrete fit examples, non-fit examples, and exact escalation rules for arch-mini-plan. Core doctrine lives in references/arch-operating-doctrine.md and references/controller-lifecycle.md. This document owns only the mini-plan-specific fit and escalation rules.
...
- Event handler rename across two call sites with one follow-up delete: still mini mode.
- Storage-layer migration plus rollout concerns plus compatibility decisions: escalate to `miniarch-step` or `arch-step`.
- Tiny UX tweak with one implementation pass: use `lilarch`.
