...
Core controller lifecycle doctrine (arm first/disarm never, ensure-install, session identity, conflict gate, staleness sweep, manual recovery) lives in [Controller Lifecycle](references/controller-lifecycle.md). arch-mini-plan's auto-plan and implement-loop controllers follow the standard arm-first pattern documented there.

## First move

Read `references/artifact-contract.md`.
Read `references/arch-operating-doctrine.md`.
Read `references/fit-and-escalation.md`.
Resolve `DOC_PATH` and read it fully.
Verify the North Star and scope are coherent enough to plan against.
Read `references/quality-bar.md`.
Read `references/one-pass-plan.md`.
Then write the canonical arch blocks in one pass.

## Workflow

Confirm this is the right mode: smaller task, still needs real architecture grounding, not a `lilarch` fit and not an immediate staged full-arch execution fit.
Gather the minimum evidence needed: internal ground truth and reusable patterns first, external guidance only if correctness depends on it.
Update the canonical blocks together: research grounding, current architecture, target architecture, call-site audit, phase plan.
Stop with a clear "ready to implement" verdict and the recommended next move.

## Output expectations

Update `DOC_PATH` only.
Keep the console summary short: North Star reminder, punchline, which blocks changed, any "too big for mini mode" warning, exact next move — usually `miniarch-step implement`, `miniarch-step reformat`, or `arch-step reformat`, with `arch-docs` as the later full-arch docs-cleanup handoff after clean code audit.

## Reference map

`references/artifact-contract.md` - canonical mini-plan artifact, required blocks, status rules, and handoff contract
`references/arch-operating-doctrine.md` - evidence, question policy, SSOT rules, and anti-fallback discipline
`references/controller-lifecycle.md` - core controller lifecycle doctrine (arm first/disarm never, ensure-install, session identity, conflict gate, staleness sweep, manual recovery)
`references/model-and-runtime.md` - model and runtime resolution for controller subprocess dispatch
