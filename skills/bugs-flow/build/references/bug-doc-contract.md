...
Bug doc contract: DOC_PATH rules, new-doc minimum contract, required blocks, status transitions, evidence rules, essential-info gate, Sentry defaults, and review rule.
...
- YAML frontmatter with: `title`, `date`, `status`, `owners`, `reviewers`, `related`
- TL;DR with: symptom, impact, most likely cause, next action, status
- analysis sections for: Bug North Star, bug summary, evidence, investigation
...
- default project: `psmobile-production` unless staging is explicitly indicated; `psmobile-staging` when staging is explicitly indicated
