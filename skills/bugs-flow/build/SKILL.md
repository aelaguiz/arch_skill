...
Read `references/bug-doc-contract.md`.
Read `references/shared-doctrine.md`.
Resolve the mode: analyze, fix, or review.
Resolve `DOC_PATH` and read it fully if it already exists.
If the user asked for fix or review but the doc is not ready, step back and repair the investigation first.
Read the mode reference and `references/quality-bar.md`.

## Workflow

Analyze mode: create or repair the bug doc. Ingest evidence and write ranked hypotheses. Mark the issue fix-ready only when the likely root cause is concrete enough to act on.
Fix mode: read the bug doc as the spec. Write or tighten the fix plan in the doc. Implement the smallest credible fix locally. Update verification, risk level, and outcome in the doc.
Review mode: only when explicitly requested by the user. Audit the implementation against the bug doc. Integrate only the feedback you agree with. If no explicit review ask exists, do not launch review side work.

## Output expectations

Update `DOC_PATH` in every mode.
Keep console output short: Bug North Star reminder, punchline, what changed, tests or evidence gathered, risks or blockers, next action.
