...
Read `references/scope-and-profile.md`.
Resolve the mode:
...
Set `LEDGER_PATH` to `.doc-audit-ledger.md` and derive any scoped working artifacts only when they actually exist.
Read `references/canonical-home-judgment.md`.
Read `references/cleanup-rules.md`.
...
For point-in-time docs older than 30 days, require the explicit code-grounded survival reason before they remain in scope as live docs.
...
`references/arch-docs-controller.md` - arch-docs auto-controller state schema and verdict source (core doctrine lives in `references/controller-lifecycle.md`)
