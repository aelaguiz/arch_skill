...
Core controller lifecycle doctrine (arm first/disarm never, ensure-install, session identity, conflict gate, staleness sweep, manual recovery) lives in [Controller Lifecycle](references/controller-lifecycle.md). `goal-loop` follows the default arm-first pattern: the parent pass writes session-scoped controller state and ends the turn; the Stop hook runs the body; the hook clears state on terminal verdict.

## First move

Read `references/controller-contract.md`.
Read `references/shared-doctrine.md`.
Resolve the mode: new or repair controller, readiness / flow-status, iterate.
Resolve `DOC_PATH` and `WORKLOG_PATH`.
Read the most recent worklog entries before choosing a new bet.
Read the matching mode reference and `references/quality-bar.md`.

## Workflow

1) Setup or repair: create or repair the controller doc and the append-only worklog. Draft the North Star, scoreboard, lever inventory, and anti-sidetrack contract. Pause for North Star confirmation when the doc is new or still draft.
2) Flow-status: if the user asks what is next, inspect the controller and worklog for readiness. Recommend the single best next move rather than reciting the whole doctrine.
3) Iterate: re-read the controller contract and the latest worklog entries. Choose one bet and pre-commit the decision rule. Do the minimum work needed for that bet. Run the smallest credible signal. Append exactly one worklog entry with evidence, learnings, and the next bet.
...
`references/arch-operating-doctrine.md` - shared planning and architecture rules across arch skills
