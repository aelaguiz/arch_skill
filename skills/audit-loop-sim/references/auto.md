...
- creates or refreshes the host-aware `audit-loop-sim` controller state
...
User-facing invocation is just `audit-loop-sim auto` in the active host runtime. If real hook support is absent or disabled, fail loud instead of pretending prompt-only repetition is the same feature.
