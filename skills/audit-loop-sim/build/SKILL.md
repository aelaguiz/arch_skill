...
Resolve repo root, root `.gitignore`, `_audit_sim_ledger.md`, and the host-aware `auto` controller state path described in `references/audit-loop-sim-controller.md`.
...
### `audit-loop-sim`-specific auto rules
...
`references/audit-loop-sim-controller.md` - audit-loop-sim controller state schema and verdict source (core doctrine lives in `references/controller-lifecycle.md`)
