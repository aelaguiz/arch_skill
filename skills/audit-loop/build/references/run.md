...
A risk front is the highest-priority unresolved problem cluster where the findings share either one critical-path story, one failure mode, or one verification surface.
...
3. Build or refresh Phase 1 exhaustively: enumerate shipped code surfaces from repo truth; enumerate the current proof surface from repo truth; record each surface's contract or invariant, downstream dependents, and consequence if it is wrong; assess current proof quality and missing signal; use churn, dead-code signals, duplication signals, and explicit fragility markers when they sharpen judgment; write map status, priorities, and explicit `SKIP` decisions.
...
12. Fix the strongest justified work across that risk front: bug fix, dead-code deletion, duplication extraction, high-value test addition.
13. Verify the initial fix: run the smallest targeted signal that proves the fix; run the broader relevant suite when that signal exists and is credible; make the proof depth proportional to the consequence and blast radius of the touched surfaces.
14. Audit the resulting diff and touched surfaces: `SAFETY` (contracts/invariants/edge handling/blast-radius containment still hold), `DOWNSTREAM` (callers/dependents/shared helpers/proof surfaces do not have unaddressed fallout), `ELEGANCE` (the fix is coherent rather than patchy/brittle/obviously awkward), `DUPLICATION` (no new duplicate logic/assertions/fallback handling was introduced).
15. If the post-change audit finds a problem, repair it in the same pass: keep the repair inside the same risk front and existing contract; allow broader same-story cleanup only when that is the cleanest way to remove the new fragility or duplication; re-run the targeted and broader proof that the repair affects.
16. Update: finding status, test additions, post-change audit status, `Last updated`, the map and ranking if the fix changed them materially, `Next Area` or `Stop Reason` if the next unfinished mapping tranche/unresolved risk front/blocker is obvious.
