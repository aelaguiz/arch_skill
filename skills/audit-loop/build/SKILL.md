...
Resolve repo root, root `.gitignore`, `_audit_ledger.md`, and the host-aware `auto` controller state path described in `references/audit-loop-controller.md`.
...
### `audit-loop`-specific auto rules
...
`references/audit-loop-controller.md` - audit-loop controller state schema and verdict source (core doctrine lives in `references/controller-lifecycle.md`)
