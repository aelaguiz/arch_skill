...
Lilarch artifact contract: DOC_PATH rules, new-doc minimum contract, WORKLOG_PATH derivation, required blocks, status rules, and fit-guard escalation.
...
- YAML frontmatter with: `title`, `date`, `status`, `owners`, `reviewers`, `fallback_policy`, `related`
- a filled `# TL;DR`
- a North Star with: claim, in scope, out of scope, definition of done
...
Derive `WORKLOG_PATH` from `DOC_PATH`: `<DOC_DIR>/<DOC_BASENAME>_WORKLOG.md`.
