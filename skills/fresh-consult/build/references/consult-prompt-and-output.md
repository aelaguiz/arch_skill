...
The consult prompt must make the child useful from a cold start. It has no session history and should not be trusted to infer the parent skill's unstated context.

## Prompt skeleton

Write a prompt like this to `prompt.md` and adapt the sections to the actual question:
...
- `pass-with-notes` - the core answer is acceptable, but the notes should be triaged.
...
`BLOCKING` issues must name the file, heading, command, claim, or checklist item that fails. For code or repo-backed consults, cite line numbers when practical.
`EVIDENCE READ` is required. A consult that does not say what it inspected is not actionable.
...
2. Quote blocking findings exactly when there are only a few; summarize only when the list is long.
...
7. For parallel groups, report each child verdict separately before writing any synthesis. Treat disagreement between children as useful signal, not a majority vote.
...
- Treat the child as final authority. It is an independent read, not a judge that overrides repo evidence.
