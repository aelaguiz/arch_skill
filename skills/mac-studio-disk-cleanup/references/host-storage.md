...
Before removing generated directories, capture `git status --porcelain=v1 -z
--untracked-files=all`, check `git ls-files` and `git check-ignore` for the exact
target, and compare status afterward. Do not delete source worktrees or Git refs.
...
| Flutter | `~/workspace/work/worktrees/`, `~/workspace/work/agent_*/worktrees/`, `~/workspace/agents/work/`, `~/workspace/agents/worktrees/`, `~/work/` | Discover checkouts within these known roots, then remove ignored, untracked `apps/flutter/build` and generated `.dart_tool` output when inactive and older than 24 hours. Under capacity pressure, recent reproducible output is also eligible once current activity is ruled out. Preserve all source and Git state. |
