...
Command-first content, non-inferable detail, passive context, and anti-patterns. Use this document when deciding what belongs inside always-on instructions, what should move out, and how to add a docs map without bloating prompt context.
...
The strongest research-backed rule is simple: Only spend `AGENTS.md` budget on information the agent is likely to miss, misinfer, or need before it can safely explore.
High-value examples:
...
Do not move repo-global rules into skills. Do not move large reusable procedures into `AGENTS.md`. Avoid self-referential routing such as telling the file to call `$agents-md-authoring` just to edit itself. `AGENTS.md` should route real repo work, not narrate or bootstrap its own authorship.
