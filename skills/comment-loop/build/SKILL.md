...
**Arm first, disarm never.** This skill is hook-owned for `auto`. Core controller lifecycle doctrine (arm first/disarm never, ensure-install, session identity, conflict gate, staleness sweep, manual recovery) lives in [Controller Lifecycle](references/controller-lifecycle.md). The rules below describe only what is specific to `comment-loop auto`. State lives at `.codex/comment-loop-state.<SESSION_ID>.json` (Codex) or `.claude/arch_skill/comment-loop-state.<SESSION_ID>.json` (Claude Code); see `references/comment-loop-controller.md` for the state schema.

## First move

Read `references/ledger-contract.md`.
Read `references/shared-doctrine.md`.
Read `references/commenting-principles.md`.
Resolve the mode: `run`, `review`, or `auto`.
Resolve repo root, root `.gitignore`, `_comment_ledger.md`, and the host-aware `auto` controller state path described in `references/comment-loop-controller.md`.
...
Keep console output short: comment North Star reminder, punchline, current mapping tranche, comment front, or verdict, evidence or tests run, next action.
...
`references/comment-loop-controller.md` - comment-loop controller state schema and verdict source (core doctrine lives in `references/controller-lifecycle.md`)
