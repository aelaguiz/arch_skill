...
Read `references/investigation-contract.md`.
Read `references/shared-doctrine.md`.
Resolve the mode: bootstrap or refresh the investigation doc, or iterate on the next hypothesis.
Resolve `DOC_PATH` and its worklog.
Re-read the North Star, scoreboard, and latest worklog before choosing the next bet.
Read the matching mode reference and `references/quality-bar.md`.

## Workflow

Bootstrap mode: create or refresh the investigation doc. Capture the North Star, scope, non-negotiables, scoreboard, ground-truth anchors, quant model / sanity checks, ranked hypotheses, and first 1-3 bets. Seed the worklog with what you inspected and what you currently believe.
Iterate mode: print a short state of the union, choose one bet with the highest information gain, pre-commit the decision rule, run the fastest brutal test that can settle the question, append the worklog entry, and update the ranked hypotheses or next bet if the evidence changed the picture.

## Output expectations

Keep the full detail in the worklog, not in console chatter.
Console output should stay short: North Star reminder, punchline, current best belief, biggest uncertainty, next bet and time budget, pointers to the doc/worklog.
