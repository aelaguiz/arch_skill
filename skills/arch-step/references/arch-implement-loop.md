...
- the host-aware implement-loop controller state path:
  ...
  - Claude Code: `.claude/arch_skill/implement-loop-state.<SESSION_ID>.json` when session id is available before the first Stop-hook turn, otherwise `.claude/arch_skill/implement-loop-state.json` as a legacy single-slot fallback until the first Stop-hook turn claims it into the session-scoped path
...
- Claude Code: prefer `.claude/arch_skill/implement-loop-state.<SESSION_ID>.json` when the session id is available before the first Stop-hook turn; otherwise create `.claude/arch_skill/implement-loop-state.json` only as a legacy single-slot fallback and let the first Stop-hook turn claim it into the session-scoped path
