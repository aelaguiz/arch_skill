...
- the host-aware implement-loop controller state path:
  ...
  - Claude Code: `.claude/arch_skill/implement-loop-state.<SESSION_ID>.json` when session id is available before the first Stop-hook turn, otherwise `.claude/arch_skill/implement-loop-state.json` until the first Stop-hook turn claims session ownership
...
- Claude Code: prefer `.claude/arch_skill/implement-loop-state.<SESSION_ID>.json` when the session id is available before the first Stop-hook turn; otherwise create `.claude/arch_skill/implement-loop-state.json` and let the first Stop-hook turn claim session ownership
