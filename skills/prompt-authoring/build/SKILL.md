...
The user does not need to name a mode, prompt type, or formal input shape. Treat casual requests like "write me a prompt that does this" as first-class prompt-authoring work: infer the prompt shape, make reasonable assumptions, write or repair the prompt, and only ask when missing information would materially change the result or create real risk.
...
Infer the job from the user's natural language: create, patch, restructure, or audit. Do not ask the user to choose a lane.
Read `references/prompt-pattern-contract.md`.
Silently identify the prompt shape or blended shapes: reusable contract, personality/collaboration, outcome-first task, evidence/retrieval, creative drafting, formatting/audience, tool-use/workflow, validation, runtime communication, or implementation planning.
Read the smallest additional reference that matches the job:
...
Lock the user's real job and desired outcome before touching wording.
Choose the lightest prompt shape that can work. Use a full sectioned contract only when durability, reuse, tools, validation, or risk justifies it.
Blend prompt-type guidance when the job needs it, such as personality plus evidence rules plus stop conditions.
Keep commander's intent and success/failure higher than process mechanics.
Make the rich sections carry real weight: system context, quality bar, output contract, evidence policy, stop rules, and error handling should teach stakes and judgment, not just fill space.
...
Run the anti-heuristic, overbuild, and hidden-context checks before returning.
